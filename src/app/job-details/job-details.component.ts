...
export class JobDetailsComponent implements OnInit {
  ...
  public checkSessionStorage(): void {
    if (!SettingsService.isServer) {
      let alreadyAppliedJobs: any = sessionStorage.getItem(
        this.APPLIED_JOBS_KEY
      );
      if (alreadyAppliedJobs) {
        let alreadyAppliedJobsArray: any = JSON.parse(alreadyAppliedJobs);
        this.alreadyApplied =
          alreadyAppliedJobsArray.indexOf(parseInt(this.id)) !== -1; // tslint:disable-line
      }
    }
  }
  ...
  private setJob(): void {
    ...
    if (res.data && res.data.length > 0) {
      ...
      this.meta.updateTag({
        name: "og:image",
        content: SettingsService.settings.companyLogoPath,
      });
      this.meta.updateTag({
        name: "og:url",
        content: `${SettingsService.urlRoot}${this.router.url}`,
      });
      this.meta.updateTag({
        name: "og:description",
